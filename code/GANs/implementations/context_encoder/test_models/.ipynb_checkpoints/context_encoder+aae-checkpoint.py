...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            ...
            nn.BatchNorm1d(512),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(512, int(np.prod(mask_shape))),
            nn.Tanh(),
        )

    def forward(self, z):
        img_flat = self.model(z)
        img = img_flat.view(img_flat.shape[0], *mask_shape)
        return img
...
os.makedirs("./Dataset/BUSI_normal/generated/Context_Encoder+AAE/", exist_ok=True)
...
def save_sample(batches_done):
    ...
    encoded_parts = encoder(masked_samples_modified)
    gen_mask = decoder(encoded_parts)
    filled_samples = masked_samples.clone()
    filled_samples[:, :, i : i + opt.mask_size, i : i + opt.mask_size] = gen_mask
    # Save sample
    sample = torch.cat((masked_samples.data, filled_samples.data, samples.data), -2)
    save_image(sample, "./Dataset/BUSI_normal/generated/Context_Encoder+AAE/%s.png" % batches_done, nrow=6, normalize=True)
...
for epoch in range(opt.n_epochs):
    ...
    for i, (imgs, masked_imgs, masked_parts) in enumerate(dataloader):
        ...
        encoded_parts = encoder(masked_imgs_modified)
        decoded_parts = decoder(encoded_parts)
        # print(valid.shape)
        # print(discriminator(gen_parts).shape)
        # Adversarial and pixelwise loss
        g_adv = adversarial_loss(discriminator(decoded_parts), valid)
        g_pixel = pixelwise_loss(decoded_parts, masked_parts)
        # Total loss
        ...
        fake_loss = adversarial_loss(discriminator(decoded_parts.detach()), fake)
