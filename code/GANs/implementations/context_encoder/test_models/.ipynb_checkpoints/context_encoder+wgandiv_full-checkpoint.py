...
os.makedirs("./Dataset/BUSI_normal/generated/Context_Encoder+WAGANDIV_FULL/", exist_ok=True)
...
def save_sample(batches_done):
    ...
    save_image(sample, "./Dataset/BUSI_normal/generated/Context_Encoder+WAGANDIV_FULL/%s.png" % batches_done, nrow=6, normalize=True)
...
for epoch in range(opt.n_epochs):
    ...
    for i, (imgs, masked_imgs, masked_parts) in enumerate(dataloader):
        ...
        # print(real_validity.requires_grad)
        # print(masked_parts.requires_grad)
        # print(real_grad_out.requires_grad)
        real_grad = autograd.grad(
            real_validity, masked_parts, real_grad_out, create_graph=True, retain_graph=True, only_inputs=True
        )[0]
        real_grad_norm = real_grad.view(real_grad.size(0), -1).pow(2).sum(1) ** (p / 2)

        fake_grad_out = Variable(Tensor(gen_parts.size(0), 1).fill_(1.0), requires_grad=False)
        fake_grad = autograd.grad(
            fake_validity, gen_parts, fake_grad_out, create_graph=True, retain_graph=True, only_inputs=True
        )[0]
