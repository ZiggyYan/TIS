...
print("TOTAL NUM OF TRAIN:",fused_train_dataset.num)
print("TOTAL NUM OF TEST:",fused_test_dataset.num)
...
# 定义文件名和内容
record_file = "./code/GANs/model_weights/discriminator/phase2-1record.txt"
...
for epoch in range(opt.n_epochs):
    ...
    #  Training
    # ----------
    for i, (imgs, masked_imgs, masked_parts, ill_states) in enumerate(train_fused_dataloader):
        ...
        print(
            "TRAINING_PART[Epoch %d/%d] [Batch %d/%d] [D loss: %f]"
            % (epoch, opt.n_epochs, i, len(train_fused_dataloader), fused_loss.item())
        )
    # ----------
    #  Testing
    # ----------
    ...
    for i, (imgs, masked_imgs, masked_parts, ill_states) in enumerate(test_fused_dataloader):
        ...
        print(
            "TESTING_PART[Epoch %d/%d] [Batch %d/%d] [D loss: %f]"
            % (epoch, opt.n_epochs, i, len(test_fused_dataloader), fused_loss.item())
        )     
