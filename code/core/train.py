...
class TrainerLite(LightningLite):
    def __init__(self, args, train_dataset, no_print=True, *vargs, **kwargs):
    ...
    def run(self, args):
        ...
        if self.is_global_zero:
            ...
            with torch.no_grad():
                # print(type(self.imgs_to_log.to(self.device)))
                # print(self.imgs_to_log.to(self.device).shape)
...
if __name__ == "__main__":
    ...
    # Data specific
    parser.add_argument("--data_path", required=True, default="./Dataset/BUSB/original")
    parser.add_argument(
    ...
    # transform_1channel = transforms.Compose(
    #     [
    #         # transforms.Grayscale(1),
    #         transforms.RandomResizedCrop(
    #             args.input_size, scale=(args.scale_min, args.scale_max), interpolation=3
    ...
    #     ]
    # )
    # "./Dataset/BUSI_normal/images"
    # dataset = build_datasets(args.data_path, transform_train)
    
    dataset = ImageDataset("./Dataset/BUSB/images", transform_initial = transform_train, mode='train')
    # print(type(dataset))
    trainer = TrainerLite(
